	...
	<c:choose>
		<c:when test="${ !empty param.keyword }">
			<sql:query var="resultset">
				SELECT fid, fname, tel, address FROM travelfood WHERE fname LIKE '%${ param.keyword }%'
			</sql:query>
		</c:when>
		<c:otherwise>
			<sql:query var="resultset">
				SELECT fid, fname, tel, address, picurl FROM travelfood LIMIT 0, 20
			</sql:query>
		</c:otherwise>
	</c:choose>
